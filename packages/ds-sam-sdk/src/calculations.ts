...
export const calcBondRiskFee = (cfg: BondRiskFeeConfig, validator: AuctionValidator): BondRiskFeeResult | null => {
  ...
  const minBidReservePmpe = cfg.minBondEpochs * revShare.expectedMaxEffBidPmpe
  const idealBidReservePmpe = cfg.idealBondEpochs * revShare.expectedMaxEffBidPmpe
  ...
  if (riskBondSol - minUnprotectedReserve < projectedProtectedStakeSol * (minBondPmpe / 1000)) {
    ...
    const base = Math.max(
      0,
      projectedProtectedStakeSol - (riskBondSol - idealUnprotectedReserve) / (idealBondPmpe / 1000),
    )
