...
describe('DataProvider.aggregateData blacklist flag handling', () => {
  ...
  it('disjoint CSV & history → correct flags from each', () => {
    const csv = ['vote_account,reason','alice,spam'].join('\n')
    const history = [{ voteAccount: 'bob', revShare: { inflationPmpe: 0, mevPmpe: 0, bidPmpe: 0, totalPmpe: 0 },
      marinadeSamTargetSol: 0, marinadeMndeTargetSol: 0, epoch: 0, values: { samBlacklisted: true } }]
    expect(extractFlags(runAggregate(csv, history))).toMatchSnapshot()
  })

  it('overlapping CSV & history → both flags true for overlapping', () => {
    const csv = ['vote_account,reason','carol,spam'].join('\n')
    const history = [{ voteAccount: 'carol', revShare: { inflationPmpe: 0, mevPmpe: 0, bidPmpe: 0, totalPmpe: 0 },
      marinadeSamTargetSol: 0, marinadeMndeTargetSol: 0, epoch: 0, values: { samBlacklisted: true } }]
    expect(extractFlags(runAggregate(csv, history))).toMatchSnapshot()
  })
})
